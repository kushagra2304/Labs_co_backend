...
class MessageReactionRepository {
    async findById(id, includeDeleted = false) {
        return prisma_config_1.default.messageReaction.findFirst({
            where: {
                id,
    ...
    async createOrRestore(messageId, userId, emoji, actorId) {
        ...
        if (existing) {
            if (existing.deletedAt) {
                return prisma_config_1.default.messageReaction.update({
        ...
        return prisma_config_1.default.messageReaction.create({
            data: {
                messageId,
                userId,
                emoji,
