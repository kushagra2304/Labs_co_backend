...
class ConversationRepository {
    async findById(id, includeDeleted = false) {
        return prisma_config_1.default.conversation.findFirst({
            where: {
                id,
    ...
    async findUserConversations(userId, limit, offset, includeDeleted = false) {
        return prisma_config_1.default.conversation.findMany({
            where: {
                ...(includeDeleted ? {} : { deletedAt: null }),
                members: {
                    some: {
    ...
    async update(id, data, actorId) {
        return prisma_config_1.default.conversation.update({
            where: { id },
            data: {
                ...data,
                updatedBy: actorId,
