...
class ConversationMemberRepository {
    async findById(id, includeDeleted = false) {
        return client_1.default.conversationMember.findFirst({
            where: {
                id,
    ...
    async findByConversation(conversationId, includeDeleted = false) {
        return client_1.default.conversationMember.findMany({
            where: {
                conversationId,
                ...(includeDeleted ? {} : { deletedAt: null }),
            },
    ...
    async addMember(conversationId, userId, actorId) {
        ...
        if (existing) {
            if (existing.deletedAt) {
                ...
                return client_1.default.conversationMember.update({
        ...
        return client_1.default.conversationMember.create({
            data: {
                conversationId,
                userId,
                createdBy: actorId,
    ...
    async removeMember(conversationId, userId, actorId) {
        const existing = await client_1.default.conversationMember.findFirst({
            where: {
                conversationId,
                userId,
                deletedAt: null,
