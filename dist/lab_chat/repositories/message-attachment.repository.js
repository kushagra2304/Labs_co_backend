...
class MessageAttachmentRepository {
    async findById(id, includeDeleted = false) {
        return client_1.default.messageAttachment.findFirst({
            where: {
                id,
    ...
    async findByMessageId(messageId, includeDeleted = false) {
        return client_1.default.messageAttachment.findMany({
            where: {
                messageId,
                ...(includeDeleted ? {} : { deletedAt: null }),
            },
    ...
    async softDeleteAndMarkHardDeleted(id, actorId) {
        return client_1.default.messageAttachment.update({
            where: { id },
            data: {
                deletedAt: new Date(),
                deletedBy: actorId,
    ...
    async softDeleteAllForMessage(messageId, actorId) {
        return client_1.default.messageAttachment.updateMany({
            where: {
                messageId,
                deletedAt: null,
            },
