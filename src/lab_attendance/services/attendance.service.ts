...
export class AttendanceService {
  constructor(private repo = new AttendanceRepository()) {}
  ...
  async addWifi(data: any, actorId: string) {
    // Validation
  ...
  async updateWifi(id: string, data: any, actorId: string) {
  ...
  async deleteWifi(id: string, actorId: string) {
  ...
  async restoreWifi(id: string, actorId: string) {
  ...
  async addHoliday(data: any, actorId: string) {
  ...
  async checkIn(userId: string, data: {
    ...
    time?: string; // Optional custom ISO timestamp
    wifiSSID?: string;
    wifiBSSID?: string;
    source: 'WiFi' | 'Manual';
    notes?: string;
    ...
    if (data.source === 'WiFi') {
      ...
      const match = wifis.find(
        (w) => w.bssid.toLowerCase() === data.wifiBSSID?.toLowerCase() && w.isActive
      );
      if (!match) {
  ...
  async checkOut(userId: string, data: {
    ...
    const updated = await this.repo.updateAttendance(attendance.id, {
      checkOut: now,
      workingHours: new Prisma.Decimal(netWorkingHours),
      breakDuration: new Prisma.Decimal(breakDuration),
  ...
  async getRequests(actorId: string, actorRole: string, filters: { userId?: string; status?: string }) {
    if (actorRole !== 'admin') {
  ...
  async processRequest(requestId: string, status: 'approved' | 'rejected', adminRemarks: string, adminId: string) {
  ...
  // Stats and Streaks
  // -------------------------------------------------------------
  async getAttendanceHistory(filters: { userId: string; startDate?: Date; endDate?: Date; status?: string }) {
    ...
    for (let d = 1; d <= currentDay; d++) {
      ...
      if (workingDaysArray.includes(policyDay)) {
        ...
        const isHoliday = holidays.some(
          (h) => h.date.toDateString() === checkDate.toDateString()
        );
        if (!isHoliday) {
          expectedDays++;
        }
  ...
  private calculateStreaks(records: any[], workingDaysArray: number[]) {
  ...
  async generateReportData(filters: {
    ...
    if (filters.department) {
      ...
      userIds = usersInDept.map((u: { id: string }) => u.id);
    }
