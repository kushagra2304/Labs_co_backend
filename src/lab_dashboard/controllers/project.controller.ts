...
function toFrontendStatus(status: string, deadline: Date | null, progress: number): string {
  if (status === "completed") return "Completed";
  if (status === "delayed") return "Overdue";
  if (deadline) {
    const daysLeft = (deadline.getTime() - Date.now()) / (1000 * 60 * 60 * 24);
    if (daysLeft < 0) return "Overdue";
    if (daysLeft <= 7 && progress < 70) return "At Risk";
  }
  return "Active";
}

function toPrismaStatus(status: string): "in_progress" | "completed" | "delayed" | "near_done" {
  switch (status) {
...
export class ProjectController {
  ...
  getProject = async (req: Request, res: Response) => {
    ...
    for (const f of project.files) {
      counts[f.fileType as keyof typeof counts]++;
      totalKb += f.sizeKb ?? 0;
    }
